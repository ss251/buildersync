...
export class MemoryManager<
    TMemory extends IMemory<any, any> = IMemory<string, any>,
  >
  extends EventEmitter<
    MemoryManagerEventMap<TMemory['type'], TMemory['content'], TMemory>
  >
  implements IMemoryManager<TMemory>
{
  /**
   ...
   */
  async addEmbeddingToMemory(memory: TMemory): Promise<TMemory> {
    return memory;
  }
  ...
  async getMemories({
    ...
    return await this.db.getMemories({
      roomId,
      count,
      unique,
  ...
  > {
    return await this.db.getCachedEmbeddings({
   ...
   * @param opts.roomId The room ID to retrieve memories for.
   * @param opts.unique Whether to retrieve unique memories only.
   * @returns A Promise resolving to an array of IMemory objects that match the embedding.
   */
  async searchMemoriesByEmbedding(
    ...
    return await this.db.searchMemories({
      tableName: this.tableName,
      roomId,
      agentId: this.runtime.agentId,
      embedding: embedding,
  ...
  async createMemory(
    ...
    await this.db.createMemory(newMemory, this.tableName, unique);
  ...
  async getMemoriesByRoomIds(params: { roomIds: UUID[] }): Promise<TMemory[]> {
    return await this.db.getMemoriesByRoomIds({
      tableName: this.tableName,
      agentId: this.runtime.agentId,
  ...
  async getMemoryById(id: UUID): Promise<TMemory | null> {
    const result = await this.db.getMemoryById<TMemory>(id);
  ...
  async removeMemory(memoryId: UUID): Promise<void> {
    await this.db.removeMemory(memoryId, this.tableName);
  }
  ...
  async removeAllMemories(roomId: UUID): Promise<void> {
    await this.db.removeAllMemories(roomId, this.tableName);
  }
  ...
  async countMemories(roomId: UUID, unique = true): Promise<number> {
    return await this.db.countMemories(roomId, unique, this.tableName);
  }
}
